    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="notifications" className="w-full">
        <TabsList className="grid w-full grid-cols-2 h-9">
          <TabsTrigger value="notifications" className="flex items-center gap-1 text-sm h-7">
            <Bell className="h-3 w-3" />
            Notificações
          </TabsTrigger>
          <TabsTrigger value="config" className="flex items-center gap-1 text-sm h-7">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-[#737373]">Nenhuma mensagem recebida ainda</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {messages
                    ...
                    .map((message: any) => (
                      <div key={message.id} className="border rounded-lg p-4 bg-blue-50 border-blue-200">
                        ...
                        <div className="text-sm text-[#737373] mb-3">
                          📞 Cliente: {message.from.replace('whatsapp:', '')} • {new Date(message.createdAt).toLocaleString('pt-BR')}
                        </div>
                        ...
                        <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
                          <p className="text-xs text-yellow-800">
                            ⚠️ Este agendamento foi solicitado via WhatsApp e precisa ser confirmado no sistema.
                          </p>
                        </div>
                      </div>
                    ))
