...
export function ProfessionalCommissionCard({
  ...
  const handleRefresh = () => {
    if (professionalId) {
      queryClient.invalidateQueries({
        queryKey: ['monthly-commission', professionalId],
      })
      queryClient.invalidateQueries({
        queryKey: ['daily-commission', professionalId],
      })
      queryClient.invalidateQueries({
  ...
  if (!professional) {
    return <p className='text-muted-foreground'>Carregando profissional...</p>
  }
    ...
    <div className='bg-background px-6 py-6 space-y-6'>
            ...
            {fetchingMonthly ? (
              ...
              <>
                ...
                <p className='text-sm text-muted-foreground'>
                  {monthlyCommission?.summary?.totalAppointments || 0}{' '}
            ...
            {fetchingDaily ? (
              ...
              <>
                <div className='text-2xl font-bold text-secondary'>
                  R${' '}
                  {dailyCommission?.summary?.totalCommission?.toFixed(2) ||
                    '0,00'}
                </div>
                <p className='text-sm text-muted-foreground'>
      ...
      <Card className='bg-card'>
        ...
        <CardContent className='grid grid-cols-7 gap-3'>
          {fetchingMonthly ? (
            <p className='text-muted-foreground col-span-7 text-center'>
              Carregando gráfico...
            </p>
          ) : (
            last7Days.map((day) => {
              const height =
                maxCommission > 0
                  ? Math.max((day.commission / maxCommission) * 100, 5)
                  : 5
              return (
                  ...
                  <div className='h-20 flex items-end justify-center mb-2'>
                    <div
                      ...
                      style={{
                        height: `${height}%`,
                        background:
                          'linear-gradient(to top, var(--color-chart-3), var(--color-chart-1))',
                      }}
                    />
                  </div>
