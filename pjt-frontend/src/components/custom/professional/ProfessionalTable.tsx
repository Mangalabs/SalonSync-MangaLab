...
import { PlusCircle, Edit, Trash2, ChevronDown, ChevronUp, Settings, Search } from 'lucide-react'
...
export function ProfessionalTable() {
  ...
  const [selectedWorkingDays, setSelectedWorkingDays] = useState<number[]>([1, 2, 3, 4, 5]) // Segunda a sexta por padrão
  ...
  const createEmployee = useMutation({
    ...
    onSuccess: () => {
      toast.success(editingProfessional ? 'Profissional atualizado com sucesso!' : 'Funcionário criado com sucesso!')
      reset()
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.message || (editingProfessional ? 'Erro ao atualizar profissional' : 'Erro ao criar funcionário'))
    },
  })
  ...
  const filteredProfessionals = professionals.filter((prof) =>
    ...
    (prof.customRole?.title || prof.role).toLowerCase().includes(searchTerm.toLowerCase()),
  )
  ...
  return (
    <div className="space-y-6">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-gray-800 truncate">{prof.name}</div>
                    <div className="flex gap-1 mt-1">
                      {daysOfWeek.map((day) => {
                        const isWorking = prof.workingDays?.some(wd => wd.dayOfWeek === day.value && wd.isActive)
                        return (
                          <span
                            key={day.value}
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      const workingDays = prof.workingDays?.filter(wd => wd.isActive).map(wd => wd.dayOfWeek) || []
      ...
      <Dialog open={!!editingProfessional || creatingNew} onOpenChange={() => { 
        setEditingProfessional(null); 
        setCreatingNew(false);
        ...
        <DialogContent className="max-w-[95vw] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{creatingNew ? 'Novo Funcionário' : 'Editar Funcionário'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {roles.map((r: any) => <SelectItem key={r.id} value={r.id}>{r.title}</SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <Button type="submit" disabled={isSubmitting} className="w-full">
              {isSubmitting ? 'Salvando...' : (editingProfessional ? 'Atualizar' : 'Criar Funcionário')}
            </Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <Dialog open={roleOpen} onOpenChange={(open) => { setRoleOpen(open); if (!open) {setEditingRole(null)} }}>
      ...
      {roles.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mt-6">
          <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
            <h3 className="text-lg font-semibold text-gray-800">Funções Criadas</h3>
          </div>
          ...
          <div
            className={`divide-y divide-gray-100 ${filteredRoles.length > 5 ? 'max-h-64 overflow-y-auto' : ''}`}
          >
            {filteredRoles.map((role: any) => (
              <div key={role.id} className="px-6 py-4 flex justify-between items-center hover:bg-gray-50 transition-colors">
                <div>
                  <div className="font-medium text-gray-800">{role.title}</div>
                  <div className="text-gray-600 text-sm">{role.commissionRate > 0 ? `${role.commissionRate}% comissão` : 'Sem comissão'}</div>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => { setEditingRole(role); setRoleOpen(true) }}
