...
export default function AppointmentHistory() {
  ...
  const filteredHistory = normalizedAppointments.filter((appointment) => {
    ...
    if (filterStartDate && appointment.date < filterStartDate) {
      return false
    }
    if (filterEndDate && appointment.date > filterEndDate) {
      return false
    }
    return true
  })
    ...
    <div className='space-y-6 p-6'>
              ...
              <thead>
                <tr className='border-b border-border'>
                  {[
                    ...
                    <th
                      key={h}
                      className='text-left py-3 px-2 font-medium text-muted-foreground text-sm'>
                      {h}
                    </th>
                  ))}
                </tr>
          ...
          <div className='bg-card rounded-2xl p-6 border border-border shadow-sm'>
            ...
            {topProfessional ? (
              <div className='text-center'>
                ...
                <h5 className='mt-3 font-semibold text-foreground'>
      ...
      <AlertDialog
        ...
        <AlertDialogContent className='bg-card text-card-foreground border border-border rounded-2xl shadow-sm'>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className='bg-muted text-muted-foreground hover:bg-hover'>
              {'Cancelar'}
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
