...
export function AppointmentStats({ appointments }: AppointmentStatsProps) {
  ...
  if (isAdmin) {
    ...
    const professionalStats = professionals.map((prof) => {
      const profAppointments = completedAppointments.filter(
        (apt) => apt.professional?.name === prof.name,
      )
      return {
        name: prof.name,
    ...
    const topProfessional = professionalStats.reduce(
  ...
  const totalRevenue = professionalAppointments.reduce(
