  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(
        `/api/appointments/queue-stats?date=${dateParam}`,
      )
      return res.data
    },
      ...
      <div className='flex justify-between items-center mb-6'>
          ...
          <div className='flex items-center gap-4 mt-1'>
            ...
            <div className='flex items-center gap-2 text-xs'>
              ...
              <span className='text-muted-foreground'>
                {realTimeUpdates.getTimeSinceLastUpdate()}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className='flex items-center space-x-2'>
          ...
          <button
            ...
            <RefreshCw
              className={`w-4 h-4 ${
                realTimeUpdates.isUpdating ? 'animate-spin' : ''
              }`}
            />
          </button>
        </div>
            ...
            <div key={i} className='bg-muted rounded-xl p-4 animate-pulse'>
              ...
              <div className='space-y-3'>
                {Array.from({ length: 3 }).map((_, j) => (
                  <div
                    key={j}
                    className='h-16 bg-muted-foreground/10 rounded'></div>
                ))}
              </div>
            </div>
          ))}
