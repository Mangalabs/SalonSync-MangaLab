...
const roleSchema = z.object({
  ...
  baseSalary: z
    ...
    .max(99999999.99, 'Salário não pode exceder R$ 99.999.999,99')
    .optional(),
  salaryPayDay: z
    .number()
...
export function RoleForm({ onSuccess, initialData }: RoleFormProps) {
  ...
  const {
    ...
    defaultValues: initialData
      ? {
          ...
          productCommissionRate: initialData.productCommissionRate || undefined,
    ...
    mutationFn: async (data: RoleFormData) => {
      try {
        ...
        if (error.response?.status === 404) {
          throw new Error(
            'Rotas /api/roles não implementadas no backend. Consulte BACKEND_IMPLEMENTATION_PRIORITY.md'
          )
        }
        throw error
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
      ...
      <div>
        <label className='block text-sm font-medium text-foreground mb-2'>
          Porcentagem de Comissão em Produtos (%)
        </label>
        <input
          type='number'
          step='0.01'
          ...
          className='w-full p-3 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-ring bg-input text-foreground'
          placeholder='Digite a porcentagem de comissão em produtos'
        />
        ...
        <p className='text-xs text-muted-foreground mt-1'>
          Deixe 0 se não houver comissão em produtos para esta função
        </p>
      </div>
