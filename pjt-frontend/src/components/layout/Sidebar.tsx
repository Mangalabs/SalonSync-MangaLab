...
const NavItem = ({ item, onClick }: { item: any; onClick: () => void }) => {
  ...
  const baseClass =
    'w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition-all duration-300 hover:translate-x-1 hover:shadow-lg'
  const gradientHover =
    'hover:bg-gradient-to-r hover:from-[var(--color-border)] hover:to-[var(--color-text-secondary)] hover:text-[var(--color-bg)]'
  return opensSaleForm ? (
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `${baseClass} ${
          isActive
            ? 'bg-gradient-to-r from-[var(--color-border)] to-[var(--color-text-secondary)] text-[var(--color-bg)] shadow-lg'
            : gradientHover
        }`
      }
      style={{ color: 'var(--color-text-secondary)' }}>
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className='hidden lg:flex w-64 h-screen flex-col fixed left-0 top-0'
        style={{
          backgroundColor: 'var(--color-sidebar)',
          color: 'var(--color-sidebar-foreground)',
          borderRight: '1px solid var(--color-sidebar-border)',
