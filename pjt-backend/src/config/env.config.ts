...
export const validateEnvironment = () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (process.env.JWT_SECRET === 'dev-jwt-secret-change-in-production-please') {
      throw new Error(
...
export const getConfig = (configService: ConfigService): EnvironmentVariables => ({
  NODE_ENV: configService.get<'development' | 'production' | 'test'>('NODE_ENV', 'development'),
  ...
  JWT_SECRET: configService.get<string>('JWT_SECRET') || 'dev-jwt-secret-change-in-production-please',
  JWT_EXPIRES_IN: configService.get<string>('JWT_EXPIRES_IN', '24h'),
  BCRYPT_ROUNDS: configService.get<number>('BCRYPT_ROUNDS', 10),

  FRONTEND_URL: configService.get<string>('FRONTEND_URL', 'http://localhost:5173'),
  ALLOWED_ORIGINS: configService.get<string>('ALLOWED_ORIGINS', 'http://localhost:3000,http://localhost:5173'),
