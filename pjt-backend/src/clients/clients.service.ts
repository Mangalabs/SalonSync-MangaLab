  ...
  async findAll(userId?: string, branchId?: string): Promise<Client[]> {
    ...
    if (userId) {
      ...
      if (user.role === 'ADMIN') {
        ...
        console.log('Found clients for professional:', clients.length, 'in branch:', professional.branchId); // Debug log
        return clients;
      }
    }
    ...
    if (targetBranchId) {
      ...
      if (user.role === 'ADMIN') {
        ...
        if (userBranches.length === 0) {
          throw new BadRequestException('Nenhuma filial encontrada para este usuário. Crie uma filial primeiro.');
        }
